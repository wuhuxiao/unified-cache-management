...
- Runtime metadata performance test: `test/test_hma_connector_runtime_perf.py`
...
- Keep `hma_connector.py` focused on runtime implementation. Do not keep production methods solely because tests use them; move view extraction, row selection, byte comparison, or pointer-inspection helpers into tests when they are not needed by the connector lifecycle.
- The scheduler hot path should use compact numpy-backed metadata (`CompactKVBlockPlan`) and avoid materializing Python `KVCacheSegment` rows except through compatibility properties used for assertions or metadata inspection.
- Allocation block-id collection is a hot path for 128 x 1M-token metadata perf. Do not assume vLLM allocation block lists expose `.take()` or any test-only batch API. If batching is needed, wrap the real vLLM sequence inside connector-owned helpers such as `BlockIdSequence.gather()` and extract only block ids required by the current compact row batch.
...
- For WA external hits, still load only the final matched boundary. Do not reintroduce scratch pointer allocation for missing WA rows; supported layouts must provide complete valid rows at the loaded/dumped boundary.
- `_trim_compact_segment_arrays()` is the Ascend hook that trims compact row offsets/lengths for non-SWA state/window groups. The base implementation is a no-op; the Ascend override must keep row byte sizes aligned with trimmed state spans such as C4/C128 tails.
...
- Avoid assuming one tensor-view block size across a group. Use `view_tensor_block_sizes`, `token_strides`, and `segment_tensor_size_list()` for segmented loads/dumps.
...
- GPU worker pointer dispatch should operate from compact row metadata. Avoid adding legacy row-selection helpers back into `hma_connector.py`; tests that need tensor contents should provide local helpers.
...
4. `request_finished_all_groups()` clears request state and returns `False`; FAWA does not currently own async block release.
...
3. `wait_for_save()` dumps both FA and WA rows for completed dump blocks and waits those dump tasks before returning; rank-0 owns dumping for MLA TP.
4. `get_finished()` and `build_connector_worker_meta()` are no-ops for the current synchronous FAWA path.
...
- If row recording produces a negative group block index at an early boundary, do not pad with `block_id=-1`. Check whether the test/config violates the supported `hash_block_size >= window_tail_tokens` assumption. Negative HBM block-id handling is not part of the supported runtime path.
- If a test needs to inspect selected FA/WA rows or tensor byte contents, keep those helpers in the test file. Do not add `extract_segment_tensor_views()`, `_select_rows()`, `_extract_group_addrs()`, or scratch-view helpers back to `hma_connector.py` just for test access.
...
- Runtime performance tests should stub store lookup/load/dump/wait and measure connector metadata work only. Cover 128 requests with 1M-token inputs for GPU and Ascend-style connectors, both prefix-hit load and chunk-prefill dump scenarios.
...
python3 -m py_compile \
  ...
  test/test_hma_connector_runtime_perf.py \
  test/test_hma_connector_ascend_tp4_e2e.py \
  test/test_hma_connector_gpu_tp4_e2e.py
