...
class KVCacheGroupLayout:
    ...
    def _build_layout(self) -> None:
        ...
        def handle_tensor(
            ...
            token_dim = 1
            tensor_block_tokens = int(t.shape[token_dim])
            if tensor_block_tokens <= 0:
                raise ValueError(f"KV cache tensor has empty block dimension: {t.shape}")
            token_strides.append(t.stride(token_dim) * t.element_size())
            tensor_sizes_per_token.append(tensor_size // tensor_block_tokens)
            tensor_block_sizes.append(tensor_block_tokens)
        ...
        self.view_meta = [
            {
                ...
                "stride": stride,
                "dtype": dtype,
                "tensor_block_tokens": tensor_block_tokens,
            }
            for name, shape, stride, dtype, tensor_block_tokens in view_meta
        ]
        logger.info(
    ...
    def _logical_to_tensor_tokens(
        self,
        logical_tokens: int,
        physical_token_block_size: int,
        tensor_block_size: int,
    ) -> int:
        scaled = logical_tokens * tensor_block_size
        if scaled % physical_token_block_size != 0:
            raise ValueError(
                f"Logical segment of {logical_tokens} tokens does not align with "
                f"tensor block size={tensor_block_size} and physical token "
                f"block size={physical_token_block_size}."
            )
        return scaled // physical_token_block_size
            ...
            [
                [
                    self._logical_to_tensor_tokens(
                        segment.offset,
                        physical_token_block_size,
                        int(tensor_block_tokens),
                    )
                    for tensor_block_tokens in self.tensor_block_sizes
                ]
    ...
    def extract_segment_tensor_views(
        ...
        for segment in segments:
            for tensor, tensor_block_tokens in zip(
                self.view_tensors, self.tensor_block_sizes
            ):
    ...
    def tensor_block_size(self) -> int:
        if len(set(self.tensor_block_sizes.tolist())) != 1:
            raise ValueError(
                "KV cache group layout has mixed tensor block token sizes: "
...
class FAWABlockSpanLayout:
    ...
    ASCEND_PHYSICAL_TOKEN_BLOCK_SPECS = {
    ...
    def __init__(
        ...
        self.group_physical_token_block_sizes = (
            self._get_group_physical_token_block_sizes()
        )
    ...
    @classmethod
    def is_ascend_kv_cache_config(cls, kv_cache_config: Optional["KVCacheConfig"]) -> bool:
        ...
        spec_names = {
            type(spec).__name__
            for group in groups
            for spec in cls.group_specs(group)
        }
    ...
    def spec_token_block_size(spec: object) -> int:
        ...
        if block_size is None:
            raise RuntimeError(
                f"FAWA KV cache spec {type(spec).__name__} has no "
                "block_size."
            )
        return int(block_size)

    @classmethod
    def spec_physical_token_block_size(cls, spec: object) -> Optional[int]:
        spec_name = type(spec).__name__
        fixed_size = cls.ASCEND_PHYSICAL_TOKEN_BLOCK_SPECS.get(spec_name)
    ...
    def _ascend_group_token_block_size(self, group_spec) -> int:
        ...
        # canonical segment inside larger physical pages. Other groups keep the
        # token block size advertised by the KV cache spec.
        if any(
            self.spec_physical_token_block_size(spec) is not None
    ...
    def _get_group_physical_token_block_sizes(self) -> tuple[int, ...]:
        physical_token_sizes: list[int] = []
        for group_id, group in enumerate(self.kv_cache_config.kv_cache_groups):
            detected = {
                physical_size
                for spec in self.group_specs(group)
                if (physical_size := self.spec_physical_token_block_size(spec))
                is not None
            }
            if len(detected) > 1:
                raise RuntimeError(
                    f"FAWA Ascend group {group_id} has mixed physical "
                    f"block sizes: {sorted(detected)}."
                )
            physical_token_sizes.append(
                detected.pop() if detected else self.group_token_block_sizes[group_id]
            )
        return tuple(physical_token_sizes)
    ...
    def block_index_to_segment(
        ...
        physical_token_block_size = self.group_physical_token_block_sizes[group_id]
        if physical_token_block_size % group_token_block_size != 0:
            raise RuntimeError(
                f"FAWA group {group_id} logical block size {group_token_block_size} "
                f"must divide physical block size {physical_token_block_size}."
            )
        blocks_per_physical = physical_token_block_size // group_token_block_size
        physical_block_idx = group_block_idx // blocks_per_physical
        if physical_block_idx != block_id:
            logger.debug(
                f"FAWA group {group_id} logical block idx {group_block_idx} "
                f"maps to physical idx {physical_block_idx}, allocated id {block_id}."
            )
        return KVCacheSegment(
            block_id=block_id,
            offset=(group_block_idx % blocks_per_physical) * group_token_block_size,
    ...
    def allocation_index(self, group_id: int, group_block_idx: int) -> int:
        return (
            ...
            * self.group_token_block_sizes[group_id]
            // self.group_physical_token_block_sizes[group_id]
        )
...
class UCMFAWAConnector(UCMDirectConnector):
    ...
    def __init__(
        ...
        logger.info(
            ...
            f"physical_token_block_sizes={self.group_physical_token_block_sizes}, "
    ...
    def _group_block_range(
        self, group_id: int, computed_end_token: int
    ) -> range:
        # The block row is determined by the logical token boundary, not by
        # how many physical HMA blocks vLLM has already allocated.
        ...
        total_group_blocks = computed_end_token // self.group_token_block_sizes[group_id]
    ...
    def _block_index_to_segment(
        ...
        if self.block_span_layout is None:
            ...
            return KVCacheSegment(
                block_id=block_id,
                offset=(group_block_idx % blocks_per_physical)
                * self.group_token_block_sizes[group_id],
    ...
    def _try_select_group_block_ids(
        ...
        for group_id in range(len(self.group_token_block_sizes)):
            ...
            for group_block_idx in group_indices:
                if self.block_span_layout is not None:
                    physical_block_idx = self.block_span_layout.allocation_index(
                        group_id,
                        group_block_idx,
                    )
                else:
                ...
                if physical_block_idx >= len(group_blocks):
                    return None
                block_id = group_blocks[physical_block_idx]
                if block_id < 0:
    ...
    def _replace_allocated_blocks(
        ...
        req_meta.allocated_group_block_ids = tuple(
            [
                -1 if block.is_null else block.block_id
    ...
    def _summarize_request_meta(self, req_meta: FAWARequestMeta) -> dict[str, object]:
        ...
        recorded_range = (
            (recorded_blocks[0], recorded_blocks[-1] + 1)
            if recorded_blocks
            else (0, 0)
        )
        sample_rows = {
        ...
        return {
            ...
            "allocated_group_lens": [
                len(group_blocks)
                for group_blocks in req_meta.allocated_group_block_ids
            ],
...
class UCMAscendFAWAConnector(UCMFAWAConnector):
    """Ascend FAWA connector with segmented physical KV block mapping."""
    ...
    def _get_group_physical_token_block_sizes(self) -> tuple[int, ...]:
        if self.block_span_layout is None:
            raise RuntimeError("Ascend FAWA connector requires block span layout.")
        return self.block_span_layout.group_physical_token_block_sizes
