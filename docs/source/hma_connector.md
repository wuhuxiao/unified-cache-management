...
`ucm/integration/vllm/hma_connector.py` 主要实现 vLLM HMA 场景下的 UCM KV cache connector。当前重点是 `UCMFAWAConnector`：它把可复用 prefix KV cache 拆成 full-attention store 和 window-attention store 两套语义，同时支持普通 GPU KV layout 和 Ascend DeepSeek V4 风格的复杂物理块映射。
...
- `block_id`: vLLM/HMA physical block id。
- `offset`: 在 physical block 内的逻辑 token offset。
...
2. 将 group logical block index 映射到 physical allocation index。
3. 将 physical block id 转成 `KVCacheSegment(block_id, offset, length)`。
...
- 对 mixed 3D tensor block sizes，不假设整个 group 的 tensor block size 相同，而是通过 `tensor_block_sizes` 和 `token_strides` 做 segment 地址换算。

GPU 中 logical block 到 physical block 的映射通常是：

```python
physical_idx = group_block_idx * group_token_block_size // group_physical_token_block_size
...
部分 compressed FA group 的 logical canonical segment 会落在更大的 physical page 中：

- `Compress4AttentionSpec`: physical 512 tokens。
- `C4IndexerSpec`: physical 4096 tokens，8 个 512-token canonical segments 可映射到同一个 physical block 的不同 offset。
- `Compress128AttentionSpec`: physical 16384 tokens，32 个 512-token canonical segments 可映射到同一个 physical block 的不同 offset。
