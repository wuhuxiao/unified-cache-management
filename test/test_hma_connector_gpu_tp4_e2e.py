    ...
    def _view_for_ptr(self, ptr: int, size: int) -> torch.Tensor:
        for base_ptr, tensor in self.by_ptr.items():
            ...
            if (
                ...
                and base_ptr <= ptr
                and ptr + size <= (base_ptr + tensor_bytes)
            ):
                offset = ptr - base_ptr
...
def test_gpu_tp4_deepseek_v4_flash_hma_e2e(tmp_path):
    ...
    assert len(producer_dispatch.dump_vllm_block_ids) == len(scheduler.group_metas)
    ...
    for idx, key in enumerate(producer_dispatch.dump_keys):
        ...
        if key == producer_dispatch.dump_keys[-1]:
            fill_ptrs(
                rank0,
                registries[0],
    ...
    assert (
        len(cast(CapturingStore, rank0.fa_store).dump_history)
        == before_rank0_dump_count
    )
    ...
    assert (
        scheduler_wrapper.build_connector_meta(
    ...
    for wrapper, worker, registry in zip(
        worker_wrappers, worker_connectors, registries
    ):
        ...
        assert all(
            history == prefix_keys[-1:] for history in wa_store.load_history[-2:]
        )
    ...
    for wrapper, worker, registry in zip(
        ...
        for request_meta in final_metadata.request_meta.values():
            ...
            if tp_dump_keys:
                ...
                fill_ptrs(worker, registry, worker.fa_group_ids, fa_ptrs, 0xA1)
                expected_fa_dump_keys.extend(tp_dump_keys)
                for key, row_bytes_value in zip(
                    tp_dump_keys,
                    ptr_row_bytes(worker, registry, worker.fa_group_ids, fa_ptrs),
                ):
            ...
            if wa_dump_ring_idx % worker.tp_size == worker.tp_rank:
                ...
                fill_ptrs(worker, registry, worker.window_group_ids, wa_ptrs, 0xD1)
    ...
    before_fa_dump_counts = [
        len(cast(CapturingStore, worker.fa_store).dump_history)
        for worker in worker_connectors
    ]
    before_wa_dump_counts = [
        len(cast(CapturingStore, worker.wa_store).dump_history)
        for worker in worker_connectors
    ]
    ...
    assert [
        key
        for worker, before_count in zip(worker_connectors, before_fa_dump_counts)
        for history in cast(CapturingStore, worker.fa_store).dump_history[before_count:]
        for key in history
    ] == expected_fa_dump_keys
    assert [
        key
        for worker, before_count in zip(worker_connectors, before_wa_dump_counts)
        for history in cast(CapturingStore, worker.wa_store).dump_history[before_count:]
        for key in history
    ] == expected_wa_dump_keys
    wait_lookup_on_prefix(
        scheduler.fa_store,
        expected_fa_dump_keys,
        len(expected_fa_dump_keys) - 1,
    )
    wait_lookup_on_prefix(
        scheduler.wa_store,
        expected_wa_dump_keys,
        len(expected_wa_dump_keys) - 1,
    )
    ...
    assert (
        scheduler_wrapper.build_connector_meta(
            scheduler_output(
                finished_req_ids={request.request_id for request in requests}
            )
        ).request_meta
        == {}
    )
