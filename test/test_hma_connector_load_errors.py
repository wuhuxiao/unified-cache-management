...
def test_first_group_anchor_ids_for_hash_range_uses_candidate_base():
    ...
    assert (
        connector._first_group_anchor_ids_for_hash_range(
            ([11, -1, 13], [21], [31]),
            4,
            5,
            3,
...
def test_start_load_kv_uses_flat_request_metadata_and_boundary_anchors():
    ...
    def extract_wa(keys, candidate_ids):
        wa_calls.append((keys, candidate_ids))
        return "wa-ptrs"
    ...
    assert wa_calls == [([b"b"], ([11, 13], [21], [31]))]
...
def test_start_load_kv_reports_wa_failure_with_final_boundary_anchor_only():
    ...
    assert connector.get_block_ids_with_load_errors() == {11, 13}
...
def test_wait_for_save_batches_flat_dump_metadata():
    ...
    def extract_wa(keys, candidate_ids):
        wa_calls.append((keys, candidate_ids))
        return np.array([[1 if keys == [b"a"] else 6]], dtype=np.uint64)
    ...
    assert connector.wa_store.dumps[0][1:3] == ((b"a", b"c"), (0, 0))
...
def test_wait_for_save_slices_tp_dump_block_ids_per_group():
    connector = object.__new__(UCMFAWAConnector)
    connector.tp_rank = 1
    connector.tp_size = 4
    connector.fa_store = RecordingStore()
    ...
    connector._extract_fa_ptr = extract_fa
    connector._extract_wa_ptr = lambda *args: np.array([[1]], dtype=np.uint64)
    ...
    assert connector.fa_store.dumps[0][1:3] == ((b"b",), (0,))
    np.testing.assert_array_equal(
        connector.fa_store.dumps[0][3],
